...
from Futilities3 import *  # your existing helper utilities
import os
import time
import scipy
...
class SimpleBlock2d(nn.Module):
    ...
    def forward(self, x):
        batchsize, size_x, size_y, C = x.shape     # fix the unpacking
        x = self.fc0(x)                            # (batch, size_x, size_y, width)
...
class Net2d(nn.Module):
    ...
    def forward(self, x):
        ...
        gridx = torch.linspace(0, 1, H, device=device).view(1, H, 1, 1).expand(B, H, W, 1)
        gridy = torch.linspace(0, 1, W, device=device).view(1, 1, W, 1).expand(B, H, W, 1)
...
class FNO2dmain():
  def __init__(self, batch_size, modes1, learning_rate, width):
  ...
  def train(self):
    ...
    results_dir = os.path.join(current_directory, "Case_FBurger_" + str(save_index))
    ...
    ################################################################
    TRAIN_PATH = '/workspace/Data/Burger/data.mat'
    TEST_PATH  = '/workspace/Data/Burger/data.mat'
    ...
    vali_loader  = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(x_vali, y_vali),
        batch_size=self.batch_size, shuffle=True)
    ...
    model = Net2d(self.modes, self.width).to(device)
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            mse = F.mse_loss(out.view(self.batch_size, -1), y.view(self.batch_size, -1), reduction='mean')
            loss = loss_fn(out.view(self.batch_size, -1), y.view(self.batch_size, -1))
            loss.backward()
            optimizer.step()
            train_ls += loss.item()
            train_mse += mse.item()
        ...
        with torch.no_grad():
            ...
            for x, y in vali_loader:
                ...
                rel_err += loss_fn(out.view(self.batch_size,-1), y.view(self.batch_size,-1)).item()
                mse=F.mse_loss(out.view(self.batch_size, -1), y.view(self.batch_size, -1), reduction='mean')
                vali_l2 += loss_fn(out.view(self.batch_size, -1), y.view(self.batch_size, -1)).item()
                vali_mse += mse.item()
                n_vali += 1
        rel_err /= ntest
    ...
    np.savetxt(save_results_to+'/epoch.txt', times)
