...
class Sumudu_Transform(nn.Module):
    def __init__(self, in_channels, out_channels, degree, modes, width, s):
        ...
        # Precompute factorials for inverse transforms
        self.factorials = torch.tensor([math.factorial(k) for k in range(degree)], dtype=torch.double)

        # Precompute Taylor coefficients (fixed functions)
        self.coeff_sin = self._taylor_coeffs("sin", degree)
        self.coeff_exp = self._taylor_coeffs("exp", degree)

    def _taylor_coeffs(self, func_name, degree):
        coeffs = []
        k = torch.arange(degree, dtype=torch.double)
        if func_name == "sin":
            coeffs = ((-1)**(2*k+1)) / torch.tensor([sp.factorial(int(2*i+1)) for i in k], dtype=torch.double)
        elif func_name == "exp":
            coeffs = 1.0 / torch.tensor([sp.factorial(int(i)) for i in k], dtype=torch.double)
        else:
            raise ValueError(f"{func_name} not supported")
        return coeffs

    def coefficient(self, input, coeff):
        scalar_input = input.mean(dim=2)  # [batch, width]
        powers = torch.arange(self.degree, device=input.device, dtype=torch.double)
        scalar_expanded = scalar_input.unsqueeze(-1) ** powers  # [batch, width, degree]
        return scalar_expanded * coeff.to(input.device).view(1, 1, -1)

    def approximate_sum(self, width, input, odd):
        t = torch.linspace(0, 1, self.s, dtype=torch.double, device=input.device)
        if odd:
            powers = torch.arange(input.shape[-1], device=input.device) * 2 +1 # [degree]
        else:
            powers = torch.arange(input.shape[-1], device=input.device) # [degree]
        t_powers = t.unsqueeze(1) ** powers
        output = torch.einsum("bwd, sd -> bws", input, t_powers)
        return output

    def transform(self, weight1, width, input):
        return torch.einsum("bim, iom -> bom", input[:, :, :self.modes1].double(), self.weight1.double())

    def inverse_transform(self, input):
        return input / self.factorials[:input.shape[-1]].to(input.device)

    def forward(self, x):
        odd = True if x in train_loader else False
        #coefficients = self.coeff_sin if x in train_loader else self.coeff_exp
        #x = self.coefficient(x, coefficients)
        x = self.transform(self.weight1, width, x)
        x = self.inverse_transform(x)
        x = self.approximate_sum(width, x, odd)
        return x
...
reader = MatReader('/content/drive/MyDrive/LNORep/1D_Duffing_c05/Data/data.mat')
