...
class Sumudu_Transform(nn.Module):
    ...
    def approximate_sum(self, width, input):
        ...
        x_lin = torch.linspace(0, self.s*0.02, self.s, dtype=torch.float64, device=self.device)  # [s]
...
class main():
    ...
    def train(self):
        ...
        # model
        model = SNO1d(self.degree, width,s).to(device)
        best_val = float("inf")
        patience = 200   # stop after 200 epochs without improvement
        counter = 0
